...
export default function R24H({ reminder, setReminder, r24hNotes, setR24hNotes, targetKcal = 0, readOnly = false }) {
  ...
  return (
    <div style={{ width: '100%' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', background: '#f9f9f9', borderRadius: '12px', padding: '0 12px', border: '1px solid #eee' }}>
              ...
              <input 
                ...
                style={{ flex: 1, padding: '12px', border: 'none', outline: 'none', fontSize: '0.85rem', background: 'none' }}
                value={searchTerm}
      ...
      <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '12px', marginBottom: '20px', WebkitOverflowScrolling: 'touch' }}>
        {meals.map(m => {
          const qty = reminder?.[m]?.length || 0;
          return (
            <button 
              key={m}
              onClick={(e) => { e.preventDefault(); setActiveMeal(m); }}
              style={{
                padding: '10px 16px',
                borderRadius: '16px',
                border: 'none',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {(!reminder?.[activeMeal] || reminder[activeMeal].length === 0) ? (
          <div className="glass-panel" style={{ padding: '32px', textAlign: 'center', background: 'rgba(0,0,0,0.01)', opacity: 0.5 }}>
            ...
            {!readOnly && <p style={{ fontSize: '0.75rem', marginTop: '4px' }}>Búscalo arriba y añádelo con un click.</p>}
          </div>
        ) : (
            ...
            <div key={entry.id} className="glass-panel" style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  ...
                  {readOnly ? (
                    <div style={{ width: '60px', padding: '6px', textAlign: 'center', fontSize: '0.9rem', fontWeight: 'bold' }}>
                      {entry.portionsQty}
                    </div>
                  ) : (
                    <input 
                ...
                {!readOnly && (
                  <button 
                    onClick={(e) => { e.preventDefault(); handleRemoveEntry(activeMeal, entry.id); }}
      ...
      <section className="glass-panel" style={{ padding: '18px', background: 'white', marginTop: '24px', marginBottom: '24px' }}>
        ...
        {readOnly ? (
          <p style={{ fontSize: '0.9rem', whiteSpace: 'pre-wrap', fontStyle: r24hNotes ? 'normal' : 'italic', opacity: r24hNotes ? 1 : 0.6 }}>
            {r24hNotes || 'Sin notas registradas.'}
          </p>
        ) : (
          <textarea
